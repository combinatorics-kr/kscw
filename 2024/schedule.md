---
layout: default
title: 2024년 조합론 학생 워크샵 – 일정
show_hero: true
hero_kicker: Korean Student Combinatorics Workshop 2024
hero_title: 2024년 조합론 학생 워크샵
hero_meta: 공주한옥마을 · 2024년 7월 29일~8월 2일
hero_year_key: 2024
hero_tab: schedule
mathjax_manual: true
---


# 일정

{% include schedule_overall_5min.html
   data = site.data.schedule.schedule_overall_2024
   slots = site.data.schedule.slots_2024 %}

## 강연 세부 정보

{% include talks_detail.html
   data = site.data.schedule.talks_detail_2024 %}